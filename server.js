...
app.post('/api/signup', async (req, res) => {
  try{
    ...
    if(String(b.passkey).length < 6) return res.status(400).json({ ok:false, error:'Passkey must be at least 6 characters.' });
...
app.post('/api/org-login', async (req, res) => {
  try{
    ...
    if(!boatName || !passkey) return res.status(400).json({ ok:false, error:'Boat name and passkey are required.' });
    ...
    if(!org || !verifyPasskey(passkey, org.passkey_hash)){
      return res.status(401).json({ ok:false, error:'Incorrect boat name or passkey.' });
    }
    const boats = await sql`SELECT id, name, is_primary, status FROM boats WHERE organization_id = ${org.id} ORDER BY created_at ASC`;
    res.json({
      ok:true,
      organization: {
